...
def main():
    ...
    parser.add_argument('--tophat_folder', required=True, dest='folder', help='mapping result folder')
    parser.add_argument('--out_folder', dest='out', default='CIRCfinder_out', help='output folder (default: CIRCfinder_out)')
    ...
    if options.debug:
        logging.basicConfig(filename='CIRCfinder.log', level=logging.DEBUG, format='%(asctime)s %(message)s')
    else:
        logging.basicConfig(filename='CIRCfinder.log', level=logging.INFO, format='%(asctime)s %(message)s')
    logging.info('CIRCfinder start')
    ...
    logging.info('Convert unmapped reads from bam to fastq')
    ...
    logging.info('Extract unmapped sequences and index')
    ...
    logging.info('Extract anchor sequences from intron regions and map')
    ...
    logging.info('index intron sequences')
    ...
    logging.info('map junction reads')
